...
(function (racine) {
  ...
  function validerContact(contact) {
    ...
    // Règle métier : un client sans montant, c'est une erreur de saisie.
    // C'est le genre de règle qu'un assistant IA n'inventera jamais tout seul :
    // elle vient du métier, pas du code.
    if (c.statut === 'client' && c.montant === 0) {
      erreurs.montant = 'Un contact au statut « client » doit avoir un montant.';
    }
